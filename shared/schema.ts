...
/**
 * LEARNED_WORDS (DEPRECATED 2026-01-12) - Words learned from various sources
 *
 * ⚠️ MIGRATION NOTICE: This table has been consolidated into vocabulary_observations.
 *
 * All queries should now use vocabulary_observations with filters:
 *   WHERE type = 'word' AND is_real_word = TRUE
 *
 * This table definition is kept for reference during the 2-week validation period.
 * A compatibility view (learned_words_compat) provides backward compatibility.
 *
 * Unique constraint on word for ON CONFLICT upsert operations
 * Enhanced with vocabulary integration tracking and basin coordinates
 *
 * @deprecated Use vocabulary_observations instead
 */
export const learnedWords_DEPRECATED = pgTable("learned_words", {
  ...
  integratedAt: timestamp("integrated_at"),
  basinCoords: vector("basin_coords", { dimensions: 64 }),
  lastUsedInGeneration: timestamp("last_used_in_generation"),
});

// For backward compatibility during migration period
export const learnedWords = learnedWords_DEPRECATED;

export type LearnedWord = typeof learnedWords.$inferSelect;
export type InsertLearnedWord = typeof learnedWords.$inferInsert;

/**
 * WORD_RELATIONSHIPS - Word co-occurrence relationships
 * Tracks which words frequently appear together for coherence boosting
 */
export const wordRelationships = pgTable("word_relationships", {
  id: serial("id").primaryKey(),
  wordA: text("word_a").notNull(),
  wordB: text("word_b").notNull(),
  coOccurrence: integer("co_occurrence").default(1),
  fisherDistance: real("fisher_distance"),
  ...
  index("idx_word_rel_word_a_phi").on(table.wordA, table.avgPhi, table.coOccurrence),
  index("idx_word_rel_high_phi").on(table.avgPhi, table.coOccurrence),
  uniqueIndex("idx_word_rel_pair").on(table.wordA, table.wordB),
]);

export type WordRelationship = typeof wordRelationships.$inferSelect;
export type InsertWordRelationship = typeof wordRelationships.$inferInsert;

/**
 * GOD_VOCABULARY_PROFILES - Per-kernel domain-specific vocabulary
 * Each kernel (Athena, Ares, Apollo, etc.) biases toward their domain words
 */
export const godVocabularyProfiles = pgTable("god_vocabulary_profiles", {
  ...
  lastUsed: timestamp("last_used").defaultNow(),
  learnedFromPhi: real("learned_from_phi"),
  basinDistance: real("basin_distance"),
}, (table) => [
  index("idx_god_vocab_god_relevance").on(table.godName, table.relevanceScore, table.usageCount),
  index("idx_god_vocab_high_relevance").on(table.relevanceScore, table.usageCount),
  uniqueIndex("idx_god_vocab_pair").on(table.godName, table.word),
]);

export type GodVocabularyProfile = typeof godVocabularyProfiles.$inferSelect;
export type InsertGodVocabularyProfile = typeof godVocabularyProfiles.$inferInsert;
