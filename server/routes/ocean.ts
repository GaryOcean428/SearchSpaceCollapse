...
oceanRouter.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const { negativeKnowledgeUnified: negativeKnowledgeRegistry } =
...
oceanRouter.get(
  ...
  async (req: Request, res: Response) => {
    try {
      const session = oceanSessionManager.getActiveSession();
      const agent = oceanSessionManager.getActiveAgent();
      const { selectMotivationMessage } = await import(
      ...
      const fullConsciousness =
        oceanAutonomicManager.getCurrentFullConsciousness();
...
oceanRouter.post(
  ...
  async (req: any, res: Response) => {
    try {
      const { injectAdminBoost, getMushroomCooldownRemaining } = await import(
        "../ocean-neurochemistry"
      );
      ...
      const duration = durationMs
        ? Math.min(300000, Math.max(1000, Number(durationMs)))
        : 60000;
      ...
      console.log(
        `[Neurochemistry] Admin boost: D+${validatedBoost.dopamine.toFixed(
          2
        )} S+${validatedBoost.serotonin.toFixed(2)} for ${duration / 1000}s`
      );
...
oceanRouter.get(
  ...
  async (req: any, res: Response) => {
    try {
      const { getActiveAdminBoost, getMushroomCooldownRemaining } =
        await import("../ocean-neurochemistry");
...
oceanRouter.post(
  ...
  async (req: any, res: Response) => {
    try {
      const { oceanAutonomicManager } = await import(
...
oceanRouter.post(
  ...
  async (req: any, res: Response) => {
    try {
      ...
      const { getMushroomCooldownRemaining } = await import(
        "../ocean-neurochemistry"
      );
      ...
      console.log(
        `[Admin] Manual mushroom cycle triggered ${
          bypassCooldown ? "(cooldown bypassed)" : ""
        }`
      );
...
oceanRouter.post(
  ...
  async (req: any, res: Response) => {
    try {
      const { injectAdminBoost, getActiveAdminBoost } = await import(
        "../ocean-neurochemistry"
      );
      ...
      const boostDuration = Math.min(
        300000,
        Math.max(1000, Number(duration) || 60000)
      );
...
oceanRouter.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `[API] /api/ocean/cycles - isInvestigating=${isInvestigating}, phi=${
          consciousness.phi?.toFixed(3) ?? "undefined"
        }, kappa=${consciousness.kappaEff?.toFixed(0) ?? "undefined"}`
      );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        success: true,
        message: wasActive
          ? "Reset trigger acknowledged - plateau counter cleared"
          : "No active reset trigger",
        wasActive,
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const resolution = Math.min(
        50,
        Math.max(5, parseInt(req.query.resolution as string) || 20)
      );
      const method =
        (req.query.method as "pca_2d" | "dim_01" | "phi_kappa") || "pca_2d";
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const count = Math.min(
        500,
        Math.max(1, parseInt(req.query.count as string) || 50)
      );
    ...
    try {
      ...
      res.json({
        ...
        strategyBreakdown: dashboard.strategies.map((s) => ({
          ...
          trend: s.recentTrend,
          trendIcon:
            s.recentTrend === "rising"
              ? "trending-up"
              : s.recentTrend === "falling"
              ? "trending-down"
              : "minus",
        })),
      });
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const maxFrames = Math.max(
        1,
        Math.min(100, parseInt(req.query.maxFrames as string) || 24)
      );
      const clusterThreshold = Math.max(
        0.05,
        Math.min(1.0, parseFloat(req.query.threshold as string) || 0.3)
      );

      const animation = geometricMemory.getClusterEvolutionFrames(
        windowSizeMs,
        maxFrames,
        clusterThreshold
      );
      ...
      res.json({
        ...
        insights: [
          animation.totalFrames === 0
            ...
            : `${animation.totalFrames} animation frames spanning ${(
                animation.timeSpanMs /
                (1000 * 60 * 60)
              ).toFixed(1)} hours`,
    ...
    try {
      ...
      if (!record) {
        return res.status(404).json({
          ...
          hint: phrase
            ? `No entry found for phrase: "${phrase.slice(0, 50)}..."`
            : `No entry found for ID: ${entryId}`,
        });
      }

      console.log(
        `[NearMiss] Conversion recorded: ${
          record.tier
        } tier, Φ=${record.phi.toFixed(4)}`
      );
...
// ═══════════════════════════════════════════════════════════════════════════
// PYTHON AUTONOMIC KERNEL INTEGRATION
...
oceanRouter.get(
  ...
  async (req: Request, res: Response) => {
    try {
      const { oceanQIGBackend } = await import("../ocean-qig-backend-adapter");
      const state = await oceanQIGBackend.getAutonomicState();
...
oceanRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const result = await oceanQIGBackend.executeSleepCycle({
...
oceanRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const result = await oceanQIGBackend.executeDreamCycle({
        basinCoords: basinCoords || Array(64).fill(0.5),
...
oceanRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const result = await oceanQIGBackend.executeMushroomCycle({
        basinCoords: basinCoords || Array(64).fill(0.5),
...
oceanRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const result = await oceanQIGBackend.recordActivityReward({
...
oceanRouter.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const result = await oceanQIGBackend.getPendingRewards(flush);
