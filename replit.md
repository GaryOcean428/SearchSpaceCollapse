...
- **HypothesisEmitter**: Bridges Python hypothesis generation to TypeScript balance checking, generating hypotheses using various strategies (e.g., BIP39 mnemonics, passphrases) with geometric priority scoring and a feedback loop. Features dynamic emission rate adjustment based on queue saturation: 5s at <5k pending, linear slowdown 5-15s at 5k-12k, steeper 15-25s at 12k-18k, max 30s at >18k. Logs queue depth and interval every 6 cycles.
...
- **Balance Feedback Loop**: Closes the loop between balance checking results and hypothesis generation. `BalanceFeedbackAnalyzer` extracts vocabulary patterns from successful balance hits (10x weight) and near-misses (HOT 5x, WARM 2x, COOL 0.5x), biasing future hypothesis generation toward historically successful word patterns. Initializes on startup with 6-hour periodic refresh cycle. Key files: `server/balance-feedback-analyzer.ts`, integration in `server/ocean-agent.ts` (generateContextualHypotheses, generateRandomPhrases).
